...
本文件定義技術型態辨識模組的資料表結構。M10 主要依賴 M06 的股價資料（daily_prices）和 M07 的技術指標資料，並新增型態辨識結果儲存表。
...
| 表名 | 說明 | 持久層 | 特殊功能 |
|-----|------|-------|---------|
| kline_pattern_results | K 線型態辨識結果表 | MyBatis | 分區、索引優化 |
| chart_pattern_results | 圖表型態辨識結果表 | MyBatis | JSONB、複雜結構 |
| trend_analysis_results | 趨勢分析結果表 | JPA + MyBatis | JSONB |
| support_resistance_levels | 支撐壓力位表 | JPA + MyBatis | 動態更新 |
| pattern_signals | 型態訊號表 | JPA + MyBatis | 索引優化 |
| pattern_statistics | 型態統計表 | MyBatis | 快取、定期更新 |

**依賴的資料表**:

| 表名 | 模組 | 用途 |
|-----|------|------|
| daily_prices | M06 | 股價 OHLCV 資料來源 |
| stocks | M06 | 股票基本資料 |
| technical_indicator_results | M07 | 技術指標（均線、ADX 等） |

---

## 2. 資料表詳細設計

### 2.1 kline_pattern_results (K 線型態辨識結果表)
...
-- 表註釋
COMMENT ON TABLE kline_pattern_results IS 'K 線型態辨識結果表（按年份分區）';
COMMENT ON COLUMN kline_pattern_results.involved_dates IS '型態涉及的交易日期陣列';
COMMENT ON COLUMN kline_pattern_results.candle_data IS 'K 線詳細資料 JSONB';
```

**JSONB candle_data 結構範例**:
...
### 2.2 chart_pattern_results (圖表型態辨識結果表)
...
-- 表註釋
COMMENT ON TABLE chart_pattern_results IS '圖表型態辨識結果表';
COMMENT ON COLUMN chart_pattern_results.key_levels IS '關鍵價位 JSONB（包含峰值、谷值、頸線等）';
COMMENT ON COLUMN chart_pattern_results.reliability_factors IS '可靠度評估因素 JSONB';
```

**JSONB key_levels 結構範例**:
...
**JSONB reliability_factors 結構範例**:
...
### 2.3 trend_analysis_results (趨勢分析結果表)
...
-- 表註釋
COMMENT ON TABLE trend_analysis_results IS '趨勢分析結果表';
COMMENT ON COLUMN trend_analysis_results.ma_alignment IS '均線排列狀態';
COMMENT ON COLUMN trend_analysis_results.structure_type IS '價格結構類型（高低點分析）';
```

**JSONB detailed_analysis 結構範例**:
...
### 2.4 support_resistance_levels (支撐壓力位表)
...
-- 表註釋
COMMENT ON TABLE support_resistance_levels IS '支撐壓力位表';
COMMENT ON COLUMN support_resistance_levels.source_type IS '識別來源類型';
COMMENT ON COLUMN support_resistance_levels.test_count IS '該價位被測試次數';
```

---

### 2.5 pattern_signals (型態訊號表)
...
-- 表註釋
COMMENT ON TABLE pattern_signals IS '型態訊號表';
COMMENT ON COLUMN pattern_signals.signal_code IS '訊號代碼（如 PTN_SIG_001）';
COMMENT ON COLUMN pattern_signals.supporting_factors IS '支持因素陣列';
```

**訊號代碼對照表**:

| signal_code | signal_name | signal_type | source_category | 預設 strength |
|-------------|-------------|-------------|-----------------|---------------|
| PTN_SIG_001 | 看漲K線型態 | BUY | KLINE | MEDIUM |
| PTN_SIG_002 | 看跌K線型態 | SELL | KLINE | MEDIUM |
| PTN_SIG_003 | 頭肩頂完成 | SELL | CHART | HIGH |
| PTN_SIG_004 | 頭肩底完成 | BUY | CHART | HIGH |
| PTN_SIG_005 | 雙重頂確認 | SELL | CHART | HIGH |
| PTN_SIG_006 | 雙重底確認 | BUY | CHART | HIGH |
| PTN_SIG_007 | 三角形突破 | BUY/SELL | CHART | MEDIUM |
| PTN_SIG_008 | 旗形突破 | BUY/SELL | CHART | MEDIUM |
| PTN_SIG_009 | 趨勢轉多 | BUY | TREND | HIGH |
| PTN_SIG_010 | 趨勢轉空 | SELL | TREND | HIGH |
| PTN_SIG_011 | 支撐確認 | BUY | SUPPORT_RESISTANCE | MEDIUM |
| PTN_SIG_012 | 壓力突破 | BUY | SUPPORT_RESISTANCE | MEDIUM |
| PTN_SIG_013 | 突破缺口 | BUY | CHART | HIGH |
| PTN_SIG_014 | 島狀反轉 | BUY/SELL | CHART | CRITICAL |

---

### 2.6 pattern_statistics (型態統計表)
...
-- 表註釋
COMMENT ON TABLE pattern_statistics IS '型態統計表（定期更新快取）';
COMMENT ON COLUMN pattern_statistics.optimal_conditions IS '最佳觸發條件 JSONB';
```

**JSONB optimal_conditions 結構範例**:
...
<mapper namespace="com.chris.fin_shark.m10.mapper.PatternAnalysisMapper">
    ...
    <select id="scanKLinePatterns" resultType="com.chris.fin_shark.m10.dto.KLinePatternScanDTO">
        ...
        LEFT JOIN daily_prices p ON k.stock_id = p.stock_id AND k.trade_date = p.trade_date
...
```
M06 資料表（資料來源）                    M10 資料表（分析結果）
┌──────────────────────┐                ┌──────────────────────┐
│    daily_prices      │ ──────────→   │ kline_pattern_results │
│ (股價 OHLCV)         │                │ (K 線型態結果)        │
└──────────────────────┘                └──────────────────────┘
           │                                       │
           │                                       ↓
           ...
           ↓                                       ↓
┌──────────────────────┐                ┌──────────────────────┐
│       stocks         │                │ trend_analysis_results│
│ (股票基本資料)        │                │ (趨勢分析結果)        │
└──────────────────────┘                └──────────────────────┘
                                                   │
M07 資料表（依賴）                                   ↓
┌──────────────────────┐                ┌──────────────────────┐
│technical_indicator_  │ ──────────→   │support_resistance_   │
│      results         │                │      levels          │
│ (技術指標結果)        │                │ (支撐壓力位)          │
└──────────────────────┘                └──────────────────────┘
                                                   │
...
| 查詢場景 | 使用索引 | 說明 |
|---------|---------|------|
| 單一股票 K 線型態查詢 | idx_kline_ptn_stock_date | 最常用查詢 |
| 全市場型態掃描 | idx_kline_ptn_date_signal | 日期 + 訊號類型 |
| 圖表型態狀態查詢 | idx_chart_ptn_stock_status | 篩選特定狀態 |
| 支撐壓力位查詢 | idx_sr_stock_active | 只查詢有效價位 |
| 型態訊號查詢 | idx_ptn_sig_date_type | 日期 + 訊號類型 |
| JSONB 關鍵價位查詢 | idx_chart_ptn_key_levels | GIN 索引 |

### 5.2 分區策略

- `kline_pattern_results` 按年份分區
- 查詢時自動裁剪不相關分區
- 歷史資料可直接 DETACH 舊年份分區進行歸檔

### 5.3 資料保留策略

| 資料表 | 保留期限 | 歸檔策略 |
|-------|---------|---------|
| kline_pattern_results | 3 年 | 年度分區歸檔 |
| chart_pattern_results | 3 年 | 按年份刪除舊資料 |
| trend_analysis_results | 1 年 | 保留最新資料 |
| support_resistance_levels | 6 個月 | 定期清理失效資料 |
| pattern_signals | 2 年 | 按狀態歸檔 |
| pattern_statistics | 永久 | 定期更新 |
...
- [M10 API規格](../specs/api/M10-API規格.md)
