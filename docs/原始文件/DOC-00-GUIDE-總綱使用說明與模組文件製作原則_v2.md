...
| 契約名稱 | 說明 | 參照章節 | 適用模組 |
|---------|------|---------|---------|
| **Signal Contract** | 統一信號格式 | 4.1 | M07, M08, M09, M10, M11, M12 |
| **API 統一規範** | Request/Response 格式、錯誤碼 | 4.4 | 所有模組 |
| **Job 模型** | Job 結構、冪等性、重試 | 4.5 | M06, M07, M08, M09, M12 |
| **資料品質規範** | 驗證規則、檢核流程 | 4.7 | M06 |
| **RBAC 權限模型** | 角色、權限、資源 | 4.6 | 所有模組 |

**使用方式**:
1. 設計 API → 參照 4.4 API 統一規範
2. 產生信號 → 參照 4.1 Signal Contract
3. 設計 Job → 參照 4.5 Job 模型
4. 驗證資料 → 參照 4.7 資料品質規範
...
1. 閱讀總綱 4.4 API 統一規範
2. 在模組文件中設計具體 API 端點
3. Response 格式寫：「遵守總綱 4.4 API 統一規範」
```

**場景 2: 產生信號**
```
1. 閱讀總綱 4.1 Signal Contract
2. 在模組文件中設計信號產生邏輯
3. 信號格式寫：「遵守總綱 4.1 Signal Contract」
```
...
| **4. API 設計** | API 端點、Request/Response | ⭐ 遵守總綱 4.4 |
| **5. 資料流設計** | 流程圖、時序圖 | ASCII 圖或文字描述 |
| **6. Job/排程設計** | Job 清單、Cron、冪等性 | ⭐ 遵守總綱 4.5 |
| **7. 業務邏輯設計** | Service 架構、演算法 | 文字描述，無程式碼 |
| **8. 整合點** | 上下游依賴、事件發布 | 模組間介面 |
| **9. 效能考量** | 效能目標、優化策略 | 參照總綱 NFR |
...
-- GIN 索引（JSONB、陣列）
CREATE INDEX idx_[表名]_jsonb ON [表名] USING GIN(jsonb_column);
```
...
**包含內容**:
- 附錄 A: 速查表（如指標清單、公式清單等）
- 附錄 B: 常見問題 FAQ
- 附錄 C: 變更歷史
- 附錄 D: 待辦事項
- 附錄 E: 參考文獻（如適用）

**重要**: 附錄不是必要章節，依模組複雜度決定是否需要。
...
**附錄使用建議**:
- 速查表：適用於有大量指標、公式、常數的模組（如 M07 技術分析、M08 基本面分析）
- FAQ：適用於易混淆或有特殊邏輯的模組
- 變更歷史：所有模組都應包含
- 待辦事項：標記未完成或需補充的部分
- 參考文獻：引用外部標準、論文、演算法時使用

**❌ 附錄不應包含**:
- 錯誤碼完整清單（錯誤碼應在第 4 章 API 設計中定義）
- 完整程式碼實作
- 重複正文內容
...
✅ 正確:
「API Response 遵守總綱 4.4 API 統一規範」
```
...
**總綱核心章節**:
- 4.1 Signal Contract → 信號必須遵守
- 4.4 API 統一規範 → API 必須遵守
- 4.5 Job 模型 → Job 必須遵守（冪等性）
- 4.7 資料品質規範 → 驗證規則

**模組文件必備產出**:
- ⭐ 第 3 章: 完整 DDL
- ⭐ 第 4 章: API 規格（遵守總綱 4.4）
- ⭐ 第 6 章: Job 設計（遵守總綱 4.5）
- ⭐ 第 7 章: 業務邏輯（文字，無程式碼）
